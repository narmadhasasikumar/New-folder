...
export const generateQr = async (payload) => {
  ...
  if (!response.ok) {
    ...
    throw new Error(body.message || "Failed to generate QR code");
  }
  return response.json();
};

export const fetchQrLocation = async (id) => {
  const response = await fetch(`${BASE_URL}/locations/qr/${id}`);
  const body = await response.json().catch(() => ({}));
  if (!response.ok || !body.qrData) {
    const error = new Error(body.message || "QR location not found");
