...
import { createLocation, deleteLocation, fetchQrLocation, generateQr } from "../services/api";
import { LocationContext } from "../context/LocationContext";

const AdminDashboard = () => {
  const { locations, refreshLocations, addLocation, markRemoved, unmarkRemoved } = useContext(LocationContext);
  ...
  const [locationQrs, setLocationQrs] = useState({});
  ...
  const handleAddLocation = async (e) => {
    ...
    try {
      ...
      const createdLocation = await createLocation({ ...form, connections });
  ...
  const handleGenerateLocationQr = async (id, label) => {
    try {
      let data;
      try {
        data = await fetchQrLocation(id);
      } catch (fetchErr) {
        if (fetchErr.status === 404 || fetchErr.message.includes("QR location not found") || fetchErr.message.includes("Failed to fetch QR location")) {
          data = await generateQr({ locationId: id, label });
        } else {
          throw fetchErr;
        }
      }

      const qrData = data.qrData || data.qrLocation?.qrData;
      if (!qrData) {
        throw new Error("No QR data returned for this location.");
      }
      setLocationQrs((prev) => ({ ...prev, [id]: qrData }));
      setMessage(`QR code loaded for ${label || id}.`);
    } catch (err) {
      setMessage(err.message);
    }
  };
    ...
    <div className="space-y-8 rounded-3xl bg-white p-6 shadow-lg">
      ...
      <form onSubmit={handleAddLocation} className="grid gap-4 rounded-3xl border border-slate-200 bg-slate-50 p-6">
        ...
        <label className="space-y-1">
          ...
          <textarea name="description" value={form.description} onChange={handleChange} rows="1" cols="50" />
        </label>
        ...
        {locations.length === 0 ? (
          ...
          <div className="space-y-3">
            {locations.map((location) => (
              <div key={location._id} className="space-y-3">
                <div className="flex flex-col gap-3 rounded-3xl border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <button
                      type="button"
                      onClick={() => handleGenerateLocationQr(location._id, location.name)}
                      className="inline-flex items-center justify-center rounded-3xl bg-slate-700 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800"
                    >
                      Show QR
                    </button>
                    <button
                ...
                {locationQrs[location._id] && (
                  <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                    <p className="mb-2 text-sm font-medium text-slate-700">QR for {location.name || location._id}</p>
                    <img className="max-h-40 max-w-full" src={locationQrs[location._id]} alt={`QR code for ${location.name || location._id}`} />
                  </div>
                )}
              </div>
            ))}
